...
const AiMessageBox = () => {
  ...
  const handleSendMessage = async (e) => {
    ...
    if (res.status === 'error') {
      ...
      dispatch(setAiPreviousValue({ name: 'chitty', message: res?.data.data }));
    }
  };
          ...
          >
            <div className={`avatar`}>
              <div className="w-12 rounded-full">
                <img
                  src={
                    darkMode ? './images/ai_black.png' : './images/ai_white.png'
                  }
                  alt="profile"
                />
              </div>
            ...
            <div>
              <h1 className="text-base">Chitty</h1>
          ...
          >
            {messages.length <= 0 ? (
              <div className="flex flex-col items-center justify-center h-[70vh]">
                <p className="text-4xl md:text-xl lg:text-4xl">Chitty</p>
                <span>Ask AI</span>
                <div className="flex items-center justify-center">
                  <img
                    src="./images/app_logo.png"
                    alt="logo"
                    className="h-[200px] w-[200px] md:h-[300px] md:w-[300px] lg:h-[250px] lg:w-[250px] opacity-25"
                  />
                </div>
              </div>
            ) : (
              messages.map((item, index) => (
                <div ref={scroll} key={index}>
                  <div
                    className={`chat ${!(item?.name === 'chitty') ? 'chat-end' : 'chat-start'}`}
                  >
                    <div
                      className={`chat-bubble ${!(item?.name === 'chitty') ? 'bg-blue-500 text-white' : darkMode ? 'bg-slate-800 text-white' : 'bg-white text-black'} shadow-md `}
                    >
                      {/* {item?.message} */}
