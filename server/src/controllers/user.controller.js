...
const deleteUserProfile = asyncErrorHandler(async (req, res) => {
  ...
  await Message.deleteMany({
    $or: [{ senderId: new mongoose.Types.ObjectId(DeleteId) }, { receiverId: new mongoose.Types.ObjectId(DeleteId) }],
  });
...
const uploadProfilePicture = asyncErrorHandler(async (req, res) => {
  ...
  if (!newResponse) {
    throw new ApiError(500, "fail", "Something went wrong while uploading the profile picture");
  }
  ...
  res.status(200).json(new ApiResponse(200, { profilePic: newResponse.profilePic }, "Profile Change Successfully"));
});
